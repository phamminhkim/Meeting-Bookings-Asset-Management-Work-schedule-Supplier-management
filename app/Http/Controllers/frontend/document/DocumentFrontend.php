...
class DocumentFrontend extends BaseController
{
    public function index(Request $request)
    {
        ...
        switch ($type) {
            ...
            case 'view':
                ...
                break;
            case 'edit':
                $this->authorize('update',  Document::findOrFail($id));
                break;
            case 'print':
                $this->authorize('view',  Document::findOrFail($id));
                break;
        ...
        switch ($doctype) {
            case Ultils::$MODULE_DOCUMENT_PDNS:
                $documentBase = new  DocumentPDNS($request);
                if ($documentBase) {
                    $form_name = $documentBase->form_name;
                }

                break;
            case Ultils::$MODULE_DOCUMENT_BGIA:
                $documentBase = new  DocumentBGIA($request);
                if ($documentBase) {
                    $form_name = $documentBase->form_name;
                }

                break;
            case Ultils::$MODULE_DOCUMENT_CHHH:
                $documentBase = new  DocumentCHHH($request);
                if ($documentBase) {
                    $form_name = $documentBase->form_name;
                }

                break;
            case Ultils::$MODULE_DOCUMENT_TKCH:
                $documentBase = new  DocumentTKCH($request);
                if ($documentBase) {
                    $form_name = $documentBase->form_name;
                }

                break;
            case Ultils::$MODULE_DOCUMENT_TKRT:
                $documentBase = new  DocumentTKRT($request);
                if ($documentBase) {
                    $form_name = $documentBase->form_name;
                }
