...
namespace App\Repositories\approve\price;
...
class ApprovePrice extends ApproveAbs
{
    ...
    public function index(Request $request)
    {
        ...
        // dd($status);
        try {
            $document = PriceReq::whereHas('approveds', function (Builder $query) use ($status, $request) {
            ...
            foreach ($document as $currentPrice) {
                //if (count($currentPayment->approveds) > 0) {
                    foreach ($currentPrice->approveds as $approve) {
                        if ($approve->user_id == auth()->user()->id && $approve->online == "X") {
                            $currentPrice->waitingApproval = new Approved();
                            $currentPrice->waitingApproval->checkout = $approve->checkout;
                            $currentPrice->waitingApproval->release = $approve->release;
                             }
                    }
                    $lastApprove = $currentPrice->approveds[count($currentPrice->approveds)-1];
                    $currentPrice->lastApprove = new Approved();
                    $currentPrice->lastApprove->created_at = $lastApprove->created_at ;
                    $currentPrice->lastApprove->checkout = $lastApprove->checkout ;
                //}
                unset($currentPrice['approveds']);
            }


            return $document->makeHidden(['purchase_note']);;;
        } catch (Exception $e) {
            $result['success'] = "0";
            return null;
        }
    ...
    public function pending_count()
    {
        ...
        try {
            $document = PriceReq::whereHas('approveds', function (Builder $query) use ($status) {
                $query->where('user_id', auth()->user()->id)
                    ->whereIn('release', $status)
                    ->where('online', 'X')
                ;
    ...
    public function notifycation(Approved $approved)
    {
       ...
       $documentType = DocumentType::find($document->document_type_id);
        ...
        if($approved->finished == 1){
            ...
            $data->url = URL('/').'/'. Ultils::$URL_PRICE_VIEW . $document->id;
            $data->object_type = PriceReq::class;
            ...
            $data->icon = "fas fa-briefcase";
            $data->content = $document->serial_num;
            $data->url = URL('/').'/'.Ultils::$URL_APPROVE_PRICE_VIEW . $document->id;
            $data->object_type = PriceReq::class;
            $data->object_id =  $document->id;
    ...
    public function update_status_agree_finish()
    {
        ...
        try {
            ...
            Log::error("Lỗi load Html tại ApprovePrice với id= " . $document->id);
