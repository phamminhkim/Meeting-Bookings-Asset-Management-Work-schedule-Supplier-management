...
namespace App\Repositories\approve\productivity;
...
class ApproveHrProductivity extends ApproveAbs
{
    ...
    public function index(Request $request)
    {
        ...
        // dd($status);
        try {
            $document = HrproductivityDoc::whereHas('approveds', function (Builder $query) use ($status, $request) {
            ...
            if ($request->filled('workshop_id')) {
                $document = $document->where('workshop_id', $request->workshop_id);
            }
            if ($request->filled('party_id')) {
                $document = $document->where('party_id', $request->party_id);
            }
            ...
            foreach ($document as $currentDoc) {
                ...
                $workshop = $workshops->where('id', $currentDoc->workshop_id)->first();
                $currentDoc->workshopName =  $workshop != null ? $workshop->name : "";
                $party = $parties->where('id', $currentDoc->party_id)->first();
                $currentDoc->partyName =  $party != null ? $party->name : "";
    ...
    public function pending_count()
    {
        ...
        try {
            $document = HrproductivityDoc::whereHas('approveds', function (Builder $query) use ($status) {
                $query->where('user_id', auth()->user()->id)
                    ->whereIn('release', $status)
                    ->where('online', 'X');
            });
    ...
    public function notifycation(Approved $approved)
    {
        ...
        $document  = new HrproductivityDoc();
        ...
        if ($approved->finished == 1) {
            ...
            $data->url = URL('/') . '/' . Ultils::$URL_HRPRODUCTIVITY_VIEW . $document->id;
            $data->object_type = HrproductivityDoc::class;
            ...
            $data->icon = "fas fa-briefcase";
            $data->content = $document->serial_num;
            $data->url = URL('/') . '/' . Ultils::$URL_APPROVE_HRPRODUCTIVITY_VIEW . $document->id;
            $data->object_type = HrproductivityDoc::class;
            $data->object_id =  $document->id;
    ...
    public function update_status_send()
    {
        ...
        //cập nhật ngày gửi
        $this->object->status = 2; //Gửi duyệt
