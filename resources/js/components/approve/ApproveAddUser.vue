    ...
    <div>
            ...
            <div class="row">
                 ...
                 <div class="col-md-6">
                  <div class="form-group row">
                    <!-- <label for="" class="col-form-label-sm col-sm-5 col-form-label  text-md-right ">{{ $t('form.document_num')}}</label> -->
                     <label   class="col-form-label-sm col-sm-4 text-md-right"  >{{$t('form.approve_confirm')}}</label>
                    <div class="col-sm-6">
                        <div class="btn-group" role="group" aria-label="User">

                            <treeselect style="font-size: 15px;" placeholder="" v-model="user_confirm" @input="addUser('1')"  :disable-branch-nodes="true"    :options="tree_user"></treeselect>
                          
                        </div>
                          <div class="d-flex justify-content-between">

                                        <ul class="list-unstyled" id="step1">
                                          <li v-for="(step,index) in team_step1_users" v-bind:key="index"
                                           class="todo-item"   draggable="true"
                                           @dragstart="dragStart(index, $event,1)"
                                            @dragenter="dragEnter"
                                            @dragend="dragEnd"
                                            @drop="dragFinish(index, $event,1)"
                                            @dragover.prevent
                                            @dragleave="dragLeave"

                                          >
                                            <span   >{{ step.username }}-{{ step.name }}</span>
                                             <span style=" float: right; color: #a45;opacity: 0.5;" class="ml-3"  @click.prevent="deleteUser(step,index,'1')">
                ...
                <div class="col-md-6">
                  <div class="form-group row">
                    <!-- <label for="" class="col-form-label-sm col-sm-5 col-form-label  text-md-right ">{{ $t('form.document_num')}}</label> -->
                     <label   class="col-form-label-sm col-sm-2 text-md-right "  >{{$t('form.approve_consider')}}</label>
                    <div class="col-sm-6">
                        <div class="btn-group" role="group" aria-label="User">

                            <treeselect style="font-size: 15px;" placeholder="" v-model="user_view" @input="addUser('2')"  :disable-branch-nodes="true"    :options="tree_user"></treeselect>
                            
                        </div>
                          <div class="d-flex justify-content-between">

                                        <ul class="list-unstyled" id="step2">
                                          <li v-for="(step,index) in team_step2_users" v-bind:key="index"
                                           class="todo-item"   draggable="true"
                                           @dragstart="dragStart(index, $event,'2')"
                                            @dragenter="dragEnter"
                                            @dragend="dragEnd"
                                            @drop="dragFinish(index, $event,'2')"
                                            @dragover.prevent
                                            @dragleave="dragLeave"

                                            >
                                             <span   >{{ step.username }}-{{ step.name }}</span>
                                             <span style=" float: right; color: #a45;opacity: 0.5;" class="ml-3"  @click.prevent="deleteUser(step,index,'2')">
                                                 <i class="fas fa-times"></i>
                                             </span>
                                            <!-- <div class="btn-group">
         ...
         <div class="form-group row">
         <label for="group_id" class="col-form-label-sm col-sm-2 text-md-right"  >{{$t('form.approve_approve')}}<small    class="text-red">( * )</small></label>
            <div class="col-sm-3">
            <div class="btn-group" role="group" aria-label="User">

                    <treeselect style="font-size: 15px;" placeholder="" v-model="user_approve"    :disable-branch-nodes="true"    :options="tree_user"></treeselect>
