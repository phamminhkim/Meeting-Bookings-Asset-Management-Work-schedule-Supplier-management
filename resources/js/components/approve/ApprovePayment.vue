                        ...
                        <b-table striped hover responsive :bordered="true" head-variant="light"  :sticky-header="false"   small
                          ...
                          <template v-slot:cell(contract_num)="data">
                             ...
                             <template v-slot:cell(serial_num)="data">
                                <span  > <a :href="viewRequest(data.item.id)"  >{{data.item.serial_num}}</a> </span>
                           </template>
                             <template v-slot:cell(content)="data">
                                <span  > <a :href="viewRequest(data.item.id)">{{data.item.content}}</a> </span>
                           </template>
...
import PayrequestTypeVue from '../category/PayrequestType.vue'
export default {
  props: {
    user_id:"",
    title:"",
  ...
  data () {
    return {
      variant_name:'approve_ApprovePayment',
      type:"PAYMENT",
      requests:[],
      document_types:[],
       total: 0,
  ...
  methods: {
    ...
    fetchRequest(page,search){
     ...
     viewRequest(id){
        return "/approve/request?type=view&id="+id;
        // window.location.href= "/approve/request?type=view&id="+id;
      },
       searchRequest(){
        //alert($('#search').val());
     ...
     getStatus(payRequest){
       ...
       if(payRequest.waitingApproval.release == '2'){
           status =  '<span class="text-success center" ><i class="fas fa-check"></i></span>' ;
       }else if(payRequest.waitingApproval.release == '1'){
      ...
      if (key == 'date_approve' || key == 'send_date_approve') {
        ...
        let b = itemB.lastApprove && item.lastApprove.checkout == undefined ? "0000-00-00" : item.lastApprove.checkout;
