              ...
              <!-- <div class="row">
                ...
                <div class="col-md-6">
                  ...
                  <div class="input-group input-group-sm mt-1">
                    ...
                    <input class="form-control form-control-navbar" id="search" type="search" v-on:keyup.enter="searchRequest()" v-on:keyup.delete="clearSearch()" v-on:search="searchRequest()" placeholder="Search" aria-label="Search">
                    <span class="input-group-append">
                      <button type="button" @click="searchRequest()" class="btn btn-default btn-flat"><i class="fas fa-search"></i></button>
                    </span>
                        ...
                        <b-table striped hover responsive :bordered="true" head-variant="light"  :sticky-header="false"   small
                          ...
                          :sort-compare="myCompare"
                           ...
                           <template  v-slot:cell(selected)="data"   >
                             ...
                             <template v-slot:cell(newtab)="data" >
                                <a target="_blank" :href="viewprice(data.item.id)" ><i title="View in new tab" class="fas fa-external-link-alt"></i></a>
                           </template>
                          <template v-slot:cell(contract_num)="data">
                             ...
                             <template v-slot:cell(serial_num)="data">
                                <span  > <a :href="viewprice(data.item.id)"  >{{data.item.serial_num}}</a> </span>
                           </template>
                             <template v-slot:cell(title)="data">
                                <span  > <a :href="viewprice(data.item.id)">{{data.item.title}}</a> </span>
                           </template>
...
export default {
  ...
  data () {
    return {
      variant_name:'approve_ApprovePrice',
      type:"PRICE",
      requests:[],
      document_types:[],
       total: 0,
  ...
  methods: {
    ...
    fetchprice(page,search){
            ...
            this.requests=Array();
             ...
             var page_url = this.page_url_approve +'?'+ params.toString()+ '&no-cache='+new Date().getTime();
     ...
     viewprice(id){
        return "/approve/price?type=view&id="+id;
        // window.location.href= "/approve/price?type=view&id="+id;
      },
       searchprice(){
        //alert($('#search').val());
        this.fetchprice(null,$('#search').val());
      },
      filter_data(){
     ...
     getStatus(priceRequest){
       ...
       if(priceRequest.waitingApproval.release == '2'){
           status =  '<span class="text-success center" ><i class="fas fa-check"></i></span>' ;
       }else if(priceRequest.waitingApproval.release == '1'){
      ...
      //     if(element.user_id == this.user_id && element.online == 'X'){
      //       if(element.release == '2'){
      //           status = 'Đã duyệt';
      //         }else if(element.release == '1'){
      //             status =  'Từ chối';
      //         }
      //     }
      //    // console.log("MY user_id"+ element.user_id+" id login ="+this.user_id+" online="+element.online);
          ...
          centered:true
            ...
            if(value){
              ...
              .then(res=>{
                ...
                if (TotalFail == 0)
                  toastr.success(this.$t('form.approved_success') + ' ' + TotalSuccess + ' trên '  + (TotalFail + TotalSuccess) + ' Duyệt Giá',"");
                else if (TotalSuccess == 0)
                  toastr.error(this.$t('form.approved_success') + ' ' + TotalSuccess + ' trên '  + (TotalFail + TotalSuccess) + ' Duyệt Giá',"");
                else
                  toastr.warning(this.$t('form.approved_success') + ' ' + TotalSuccess + ' trên '  + (TotalFail + TotalSuccess) + ' Duyệt Giá',"");
      ...
      if (key == 'date_approve' || key == 'send_date_approve') {
        ...
        let a = itemA.lastApprove && item.lastApprove.checkout == undefined ? "0000-00-00" : item.lastApprove.checkout;
        let b = itemB.lastApprove && item.lastApprove.checkout == undefined ? "0000-00-00" : item.lastApprove.checkout;// Split them into an array of parts (yyyy-mm-dd)
        a = a.split('-')
        b = b.split('-')
        // convert string parts to numbers
        a = (parseInt(a[0], 10) * 10000) + (parseInt(a[1], 10) * 100) + parseInt(a[2])
