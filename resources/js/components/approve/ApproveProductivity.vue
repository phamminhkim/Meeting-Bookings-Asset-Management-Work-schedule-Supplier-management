               ...
               <div class="row" v-if="show_search" style="border: 1px solid #E5E5E5;border-radius:5px;">
                  <div class="col-md-12 ">
                      ...
                      <div class="form-group row">
                        ...
                        <label  class="col-form-label-sm col-sm-2 col-form-label text-left  text-md-right"   for="id_bophan">{{$t('form.workshop')}} &nbsp; &nbsp; &nbsp; </label>
                        <div class="col-sm-2">
                          <select name="" id="id_bophan" v-model="form_filter.workshop_id" class="form-control form-control-sm mt-1">
                            <option
                                v-for="workshop in workshop_filter"
                                :key="workshop.id"
                        ...
                        <label  class="col-form-label-sm col-sm-2 col-form-label text-left  text-md-right"   for="id_bophan">{{$t('form.party')}} &nbsp; &nbsp; &nbsp; </label>
                        <div class="col-sm-2">
                          <select name="" id="id_bophan" v-model="form_filter.party_id" class="form-control form-control-sm mt-1">
                            <option
                                v-for="party in party_filter"
                                :key="party.id"
                          ...
                          :sort-compare="myCompare"
                           ...
                           <template  v-slot:cell(selected)="data"   >
                             ...
                             <template v-slot:cell(newtab)="data">
                                <span  > <a  href="#" @click.prevent="viewNewTab(data.item.id)"  ><i title="View in new tab" class="fas fa-external-link-alt"></i></a> </span>
                           </template>
                           ...
                           <template v-slot:cell(status)="data">
                                ...
                                <span  v-if="data.value == 5" class="badge bg-success"> {{$t('Đã hoàn thành')}}</span>
                                <span  v-else-if="data.value == 4" class="badge bg-primary"> {{$t('Duyệt hoàn tất')}}</span>
                                <span  v-else-if="data.value == 3" class="badge bg-primary"> {{$t('Phản hồi')}}</span>
                                <span  v-else-if="data.value == 2" class="badge bg-primary"> {{$t('Đã gửi duyệt')}}</span>
                                <span  v-else-if="data.value == 1" class="badge bg-primary"> {{$t('Đã nhập dữ liệu')}}</span>
...
export default {
  ...
  data () {
    return {
      variant_name:'approve_ApproveProductivity',
      type:"HR",
      requests:[],
      document_types:[],
       total: 0,
    ...
    fetchDocument(page,search){
            ...
            this.requests=Array();
              const params = new URLSearchParams({
                ...
                workshop_id:     this.form_filter.workshop_id,
                party_id:       this.form_filter.party_id,
     ...
     viewNewTab(id){
        window.open("/approve/productivity?type=view&id="+id, '_blank');
        
      },
     viewDocument(id){
       
        window.location.href= "/approve/productivity?type=view&id="+id;
      },
       searchDocument(){
        //alert($('#search').val());
     ...
     rows(){
        ...
        vendor_filter(){
         ...
         workshop_filter(){
          let department_id = this.form_filter.department_id;
         // this.contract.department_id = "";
         ...
         party_filter(){
          let workshop_id = this.form_filter.workshop_id;
         // this.contract.department_id = "";
