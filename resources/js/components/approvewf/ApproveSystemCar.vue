        ...
        <div class="card">
            ...
            <div class="row align-items-center how-it-works d-flex" v-for="(step, index) in info.info_user_approve"
              ...
              <p style="padding-top: 5px;">
                ...
                >
                    <button
                        v-if="(workflowstep.step === 1 && info.is_step_exist.includes(1)===false) || (workflowstep.step === 1 && info.info_user_approve[0].release===-2)"
                        type="button"
                    ...
                    <button
                        v-if="(workflowstep.step === 2 && info.is_checkout_step1===true && info.is_cause===true && info.is_step_exist.includes(2)===false) || ( info.is_checkout_step1===true && workflowstep.step === 2 && info.is_refuse_step2===true && info.is_cause===true) "
                        type="button"
                        class="btn btn-success  btn-block"
                        @click="
                            showDialogChooseApprove(
                    ...
                    <button
                        v-if="(workflowstep.step === 3 && info.is_step_exist.includes(2)===true && step_apps!=2 && info.is_monitor===true  && info.is_evaluation===true && info.is_step_exist.includes(3)===false) || ( info.is_refuse_step2===false && workflowstep.step === 3 && info.is_refuse_step3===true && info.is_evaluation===true )"
                        type="button"
                        class="btn btn-success  btn-block"
                        @click="
                            showDialogChooseApprove(
            ...
            <div class="card-footer" v-if="!info.is_car_cancel">
                ...
                <button v-if="info.is_approve && step_apps===1"
                    type="button"
                    class="btn btn-success  btn-block"
                    :disabled="loading"
                      @click="
                ...
                <button
                    ...
                    class="btn btn-success  btn-block"
                    :disabled="loading"
                    v-if="info.is_approve && step_apps===2"
                    @click="approveCarAgree()"
                >
                    <i class="fas fa-check"> </i> {{ $t("form.approve") }}
                </button>
                <button v-if="info.is_approve && step_apps===3"
                    type="button"
                    class="btn btn-success  btn-block"
                    :disabled="loading"
                      @click="
        ...
        <div class="modal fade" id="mucdoloi" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="mucdoloi">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                    <div v-for="carerror in this.carerrors"
                                            :key="carerror.id">
                        <div class="custom-control custom-radio">
                          <input class="custom-control-input mucdoloiapdung" type="radio" name="car_error_id" v-model="car_error_id" :id="carerror.id" v-bind:value="carerror.id" required>
...
export default {
    ...
    data() {
        return {
            ...
            page_url_wfstep: "api/category/get_workflow/?code=SCAR",
                   ...
                   // console.log(data);
                    ...
                    if (data.data.success == "1") {
                      ...
                      //  }
                        ...
                        //toastr.error(this.$t("form.send_approved_error"), "Người duyệt không được trống.");
                    }
                })
                .catch(err => {
                    this.loading = false;
