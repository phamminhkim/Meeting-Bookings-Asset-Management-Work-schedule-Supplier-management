<template>
    <div class="container ">
        ...
        <div class="form-group d-flex align-items-center" v-if="asset_record.assetdable_id">
            Tên:
            <div class="font-weight-bold flex-grow-1  ml-1" style="border-bottom: 1px dotted #cccccc;">
                {{ asset_record.assetdable.name }}
            </div>
        </div>
        <div class="form-group">
            <div class="form-group d-flex align-items-center">
                Lý do:
                <div :class="asset_record.note !== null ? 'flex-grow-1  ml-1 font-weight-bold ' : 'mt-3 flex-grow-1 font-weight-bold  ml-1'"
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in assetItems" :key="index">
                            ...
                            <td v-if="item.objectable_id && asset_record.trans_type == 1"> {{ item.objectable.description }}
                            </td>
                            <td v-if="asset_record.asset_transaction_items.length !== 0 && asset_record.trans_type == 4"> {{ item.history.description }}
                            </td>
                            <td></td>
                        </tr>
                    </tbody>
        ...
        <div class="form-group font-italic row">
            <div class="col-md-6">
                <div class="text-center">
                    <label>Ngày xác nhận: </label> <span class="text-secondary" v-if="asset_record.confirm == 1">{{
                        asset_record.updated_at | formatDate }}</span>
                </div>
            </div>
            <div class="col-md-6">
                <div class="text-center">
                    ...
                    <span class="text-secondary">{{ asset_record.date_transaction | formatDate }}</span>
                </div>
            </div>
        </div>
                ...
                <thead>
                    <tr style="height:150px;">
                        <td>
                            ...
                            <label class="mt-5" v-if="asset_record.user_id && asset_record.confirm == 1"> {{
                                asset_record.user.name }} </label>
...
export default {
    ...
    methods: {
        fetchAssetMy() {
            ...
            if (this.id != '') {
                this.loading = true;
                var page_url = this.url_asset_my + "/" + this.id;
