    ...
    <div>
      <div class="content-header">
        <div class="container-fluid">
          <div class="row">
            <div class="col-sm-6">
              <h6 class="m-0 text-dark" style="padding-top: 10px;"><i class="fa fa-angle-double-right fa-xs" style="color: #96989a;"></i> {{$t('form.statistical')}}</h6>
            </div>
            <div class="col-sm-6">
          ...
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box">
              ...
              <div class="info-box-content">
                <span class="info-box-text">{{$t('form.new')}}</span>
                <span class="info-box-number">
                 {{this.statistics.is_new}} 
                </span>
              </div>
              <!-- /.info-box-content -->
            </div>
          ...
          <!-- /.col -->
            ...
            <div class="info-box mb-3">
              ...
              <div class="info-box-content">
                <span class="info-box-text">{{$t('form.pendding')}}</span>
                <span class="info-box-number"> {{this.statistics.is_pendding}} </span>
              </div>
              <!-- /.info-box-content -->
            </div>
            <!-- /.info-box -->
          ...
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box mb-3">
              <span class="btn info-box-icon bg-success elevation-1"><i class="fa fa-check-circle"  @click="fetchCarStatus(2)"></i></span>

              <div class="info-box-content">
                <span class="info-box-text">{{$t('form.finished')}}</span>
                <span class="info-box-number">{{this.statistics.is_finished}} </span>
              </div>
              <!-- /.info-box-content -->
            </div>
            <!-- /.info-box -->
          ...
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box mb-3">
              <span class="btn info-box-icon bg-danger elevation-1"><i class="fa fa-times"  @click="fetchCarStatus(-2)"></i></span>

              <div class="info-box-content">
                <span class="info-box-text">{{$t('form.denied')}}</span>
                <span class="info-box-number">{{this.statistics.is_denied}}</span>
              </div>
              <!-- /.info-box-content -->
            </div>
            <!-- /.info-box -->
                  ...
                  <div class="row">
                        ...
                        <b-table striped hover responsive  :sticky-header="false" 
                           ...
                           <template #cell(status)="data">
                               ...
                               <span v-else-if="data.value == -10" class="badge badge-dark">{{$t('Ẩn')}} </span>
...
export default {
  ...
  data () {
    return {
     fileName:"Danh_sach_"+moment(Date()).format('DD/MM/YYYY'),
     tableData: [],
     car:0,
      fields1: {
                 "STT": "STT",
                 "Người tạo": "Người_tạo",
                 "Nhân viên QA": "Nhân_viên_QA",
                 "Ngày phát hành phiếu": "Ngày_phát_hành_phiếu",
                 "Ngày tới hạn": "Ngày_tới_hạn",
                 "Ngày nhập nguyên nhân": "Ngày_nhập_nguyên_nhân",
                 "Số lần tái diễn": "Số_lần_tái_diễn",
                 "Số phiếu": "Số_phiếu",
                 "Công ty": "Công_ty",
                 "Phòng ban": "Phòng_ban",
                 "Sự không phù hợp": "Sự_không_phù_hợp",
                 "Mức độ lỗi": "Mức_độ_lỗi",
                 "Trạng thái": "Trạng_thái",
                 "Loại phiếu":'Loại_phiếu',
                 "Loại phiếu car hệ thống":'Loại_phiếu_car_hệ_thống',
                 "Tiêu chuẩn":'Tiêu_chuẩn',
                 "Xử lý tức thời": 'Xử_lý_tức_thời',
                 "Thời hạn": 'Thời_hạn',
                 "Người phụ trách": 'Người_phụ_trách',
                 "Nguyên nhân": 'Nguyên_nhân',
                 "Biện pháp khắc phục": 'Biện_pháp_khắc_phục',
                 "Thời gian thực hiện": 'Thời_gian_thực_hiện',
                 "Người phụ trách nguyên nhân": 'Người_phụ_trách_nguyên_nhân',
                 "Kết quả giám sát thực hiện": 'Kết_quả_giám_sát_thực_hiện',
                 "Ngày giám sát thực hiện": 'Ngày_giám_sát_thực_hiện',
                 "Nội dung đánh giá": 'Nội_dung_đánh_giá',
                 "Kết quả đánh giá": 'Kết_quả_đánh_giá',
                 "Ngày đánh giá": 'Ngày_đánh_giá',
      },
  ...
  created() {
    ...
    filter_data(){
     ...
     init(){
       ...
       fetStatistics() {
        // const this.token = "Bearer " + window.Laravel.access_this.token;
        var page_url = this.page_url_statistic;
        fetch(page_url, { headers: { Authorization: this.token } })
            .then(res => res.json())
            .then(res => {
      ...
      fetchRequest(page,search){
            ...
            .then(res=>{
               ...
               for (let i = 0; i <this.requests.length ; i++) {
                  var fast_process="";
                  var date_fast_process="";
                  var person_in_charge_fast_process="";
                 if(this.requests[i].fast_process.length>0 && this.requests[i].document_type.code==='PCAR'){
                   for(let $j=0;$j<this.requests[i].fast_process.length;$j++){
                      ...
                      fast_process += "<br/>"+this.requests[i].fast_process[$j].content.replace(/<\/?[^>]+(>|$)/g, "");
                      date_fast_process += "<br/>"+moment(this.requests[i].fast_process[$j].date).format('DD/MM/YYYY');
                      person_in_charge_fast_process += "<br/>"+this.requests[i].fast_process[$j].person_in_charge.replace(/<\/?[^>]+(>|$)/g, "");
                   }
                 }
                  if(this.requests[i].cause_measure.length>0){
                      ...
                      var is_date_cause ="";
                      var is_create_cause = "";
                      var person_in_charge_cause = "";
                   for(let $j=0;$j<this.requests[i].cause_measure.length;$j++){
                     cause += "<br/>"+this.requests[i].cause_measure[$j].cause;
                     is_measure += "<br/>"+this.requests[i].cause_measure[$j].measure;
                     is_date_cause += "<br/>"+moment(this.requests[i].cause_measure[$j].date).format('DD/MM/YYYY');
                     is_create_cause += "<br/>"+moment(this.requests[i].cause_measure[$j].date).format('DD/MM/YYYY');
                     person_in_charge_cause += "<br/>"+this.requests[i].cause_measure[$j].person_in_charge.replace(/<\/?[^>]+(>|$)/g, "");
                   }
                 }
                 //console.log(this.measure);
                   ...
                   for(let $j=0;$j<this.requests[i].monitor_implementation.length;$j++){
                      is_date_monitor += "<br/>"+moment(this.requests[i].monitor_implementation[$j].date).format('DD/MM/YYYY');
                     if(this.requests[i].monitor_implementation[$j].result=='0')
                     monitor_implementation += "<br/>"+'Hoàn thành';
                     else
                      monitor_implementation += "<br/>"+'Chưa hoàn thành';
                   }
                 }
                  if(this.requests[i].result_evaluation.length>0){
                   ...
                   for(let $j=0;$j<this.requests[i].result_evaluation.length;$j++){
                       content_evaluation += "<br/>"+this.requests[i].result_evaluation[$j].content.replace(/<\/?[^>]+(>|$)/g, "");
                       is_date_result += "<br/>"+moment(this.requests[i].result_evaluation[$j].date).format('DD/MM/YYYY');
                     if(this.requests[i].result_evaluation[$j].result=='0')
                     result_evaluation += "<br/>"+'Đạt';
                     else if(this.requests[i].result_evaluation[$j].result=='1')
                     result_evaluation += "<br/>"+'Không đạt';
                     else
                     result_evaluation += "<br/>"+'Không có dữ liệu';
                   }
                 }
                  var is_status ='';
                 ...
                 var is_date= moment(this.requests[i].issue_date).format('DD/MM/YYYY');
                 
                 var is_date_to_limit = "";
                 if(this.requests[i].date_to_limit){
                  is_date_to_limit = moment(this.requests[i].date_to_limit).format('DD/MM/YYYY');
                 }else{
                   is_date_to_limit = "";
                 }
                 ...
                 var standard= "";
                 if(this.requests[i].standard_id !=null)
                 standard = this.requests[i].standard.name;
               ...
               // console.log(this.requests[i].car_error.name);
                list.push({
                  ...
                  Phòng_ban: this.requests[i].department.name,
                  ...
                  Thời_hạn: date_fast_process,
                  Người_phụ_trách: person_in_charge_fast_process,
                  Nguyên_nhân: cause,
                  Biện_pháp_khắc_phục: is_measure,
                  Thời_gian_thực_hiện: is_date_cause,
                  Người_phụ_trách_nguyên_nhân: person_in_charge_cause,
      ...
      fetchCarStatus(status){
            ...
            .then(res=>{
               ...
               for (let i = 0; i <this.requests.length ; i++) {
                 ...
                 var is_date ="";
                 is_date = moment(this.requests[i].issue_date).format('DD/MM/YYYY');
                 var is_date_to_limit = "";
                 if(this.requests[i].date_to_limit){
                  is_date_to_limit = moment(this.requests[i].date_to_limit).format('DD/MM/YYYY');
                 }else{
    ...
    //window.location.href= "payment/requests?type=view&id="+id;
      return "car/statistical?type=view&id="+id;
    },
    // printRequest(){
    //     if(this.selected.length != 1){
