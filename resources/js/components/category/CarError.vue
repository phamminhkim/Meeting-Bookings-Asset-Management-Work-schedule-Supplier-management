                    ...
                    <form @submit.prevent="AddWorkLow" @keydown="clearError">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control form-control-sm" v-model="carerror.active" name="active" id="active">
  ...
  props: {
    ...
    data() {
        return {
            ...
            edit: false,
            token:"",
            page_url_carerrors : "/api/category/carerrors",
    ...
    methods: {
        ...
        AddWorkLow() {
            ...
            if(this.edit === false){
                ...
                fetch(page_url, {
                method: "POST",
                body: JSON.stringify(this.carerror),
                headers: {
                ...
                //update
                 fetch(page_url+"/"+this.carerror.id, {
                method: "PUT",
                body: JSON.stringify(this.carerror),
                headers: {
                    Authorization: this.token,
                ...
                .then(data => {
                    ...
                    if (!data.data.errors) {
                       ...
                       // this.carerrors[this.carerror.index]=this.carerror;
                        this.$set(this.carerrors, this.carerror.index, {...this.carerror});
        ...
        editWorkLow(carerror){

         let index = this.carerrors.findIndex(i => {
            console.log("id"+i.id);
            return i.id == carerror.id;
            });
           // var carerror={..._obj};
            this.edit = true;
            this.carerror.id            = carerror.id;
            this.carerror.name    = carerror.name;
            this.carerror.description   = carerror.description;
            this.carerror.active        = carerror.active;
        ...
        assignUserShow(carerror){
            return '/category/carerrorstep?type=assign&id='+carerror.id;
        },

        select() {
        this.selected = [];
        if (this.selectAll) {
          for (let i in this.carerrors) {
            this.selected.push(this.carerrors[i].id);
          }
        }
      },
      filter_data(){
      ...
      clearFilter(){
        ...
        reset(){
            this.clearAllError();
            this.edit = false;
            for(let field in this.carerror){
                this.carerror[field] = null;
            }
        },
        clearAllError(){
            this.errors = {};
    ...
    computed: {
     ...
     department_filter(){
        ...
        department_filter_form(){
          let company_id = this.carerror.company_id;
         // this.contract.document_type_id = "";
          return this.departments.filter(function(item){
              if(item.company_id == company_id){
                return true;
