                  ...
                  <div class="form-group row">
                    <b-table hover responsive :bordered="true"   head-variant="light" sticky-header="true" small
                        ...
                        >
                          ...
                          <template #head(type_rank)>
                            <span>Xếp loại<small class="text-red">(*)</small></span>
                         </template>
                         
                         <template #head(value)>
                            <span>Giá<small class="text-red">(*)</small></span>
                         </template>
                        ...
                        <template #cell(value)="data">
                             <vue-numeric v-bind:precision="1" :separator="separator" de  v-model="data.item.value" style="text-align:right" class="form-control form-control-sm"></vue-numeric>
                            <!-- <input v-model="data.item.from" type="number" style="text-align:right"  class="form-control form-control-sm"> -->
                        </template>
                           
                        <template #cell(type_rank)="data">
                          <input  class="form-control form-control-sm"  v-model="data.item.type_rank">                            
                        </template>
...
export default {
  ...
  data(){
    return{
      ...
      token:"",
      errors:[],        
      page_url_hrconfigprice:"api/category/hrconfigprice",
      page_url_company:"/api/category/companies",
      hrconfigtprice_index_form:"api/category/hrconfigprice_index_form",
  ...
  methods: {
    ...
    save(){
          this.loading = true;
          var page_url = this.page_url_hrconfigprice;         
    ...
    fetchTypeLimit(){
      ...
      const params = new URLSearchParams(
        ...
        var page_url = this.hrconfigtprice_index_form +"?"+params.toString();
        fetch(page_url,{
            method:'get',
            headers:{
                Authorization:this.token,
