...
<div>
    ...
    <div class="row">
        ...
        <!-- Modal -->
        <div class="modal fade" id="AddTypeCar" tabindex="-1" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form @submit.prevent="AddTypeCar" @keydown="clearError">
                        <div class="modal-header">
                            <h4 class="modal-title">{{$t('form.add')}} Lỗi phiếu car </h4>
                            <button
                                type="button"
                                class="close"
                                data-dismiss="modal"
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control form-control-sm" v-model="typecar.active" name="active" id="active">
  ...
  props: {
    ...
    data() {
        return {
            ...
            edit: false,
            token:"",
            page_url_carerrors : "/api/category/typecars",
    ...
    methods: {
        ...
        AddTypeCar() {
            ...
            if(this.edit === false){
                ...
                fetch(page_url, {
                method: "POST",
                body: JSON.stringify(this.typecar),
                headers: {
                ...
                .then(data => {
                    ...
                    if (!data.data.errors) {
                         this.reset();
                        this.typecars.push(data.data);
                        this.showMessage('Thông báo','Lưu thành công');
                        $("#AddTypeCar").modal("hide");
                        this.fetchCarerror();
                ...
                //update
                 fetch(page_url+"/"+this.typecar.id, {
                method: "PUT",
                body: JSON.stringify(this.typecar),
                headers: {
                    Authorization: this.token,
                ...
                .then(data => {
                    ...
                    if (!data.data.errors) {
                       ...
                       // this.typecars[this.typecar.index]=this.typecar;
                        this.$set(this.typecars, this.typecar.index, {...this.typecar});
        ...
        editWorkLow(typecar){
         ...
         let index = this.typecars.findIndex(i => {
            console.log("id"+i.id);
            return i.id == typecar.id;
            });
           // var typecar={..._obj};
            this.edit = true;
            this.typecar.id            = typecar.id;
            this.typecar.name    = typecar.name;
            this.typecar.description   = typecar.description;
            this.typecar.active        = typecar.active;
        ...
        assignUserShow(typecar){
            return '/category/carerrorstep?type=assign&id='+typecar.id;
        },

        select() {
        this.selected = [];
        if (this.selectAll) {
          for (let i in this.typecars) {
            this.selected.push(this.typecars[i].id);
          }
        }
      },
      filter_data(){
      ...
      clearFilter(){
        ...
        reset(){
            this.clearAllError();
            this.edit = false;
            for(let field in this.typecar){
                this.typecar[field] = null;
            }
        },
        clearAllError(){
            this.errors = {};
    ...
    computed: {
     ...
     department_filter(){
        ...
        department_filter_form(){
          let company_id = this.typecar.company_id;
         // this.contract.document_type_id = "";
          return this.departments.filter(function(item){
              if(item.company_id == company_id){
                return true;
