                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <b-table striped hover bordered responsive head-variant="light" small :fields="fields"
                                :items="workflowTypes" :filter="filter" :current-page="current_page"
                                :per-page="per_page">
                    ...
                    <form @submit.prevent="save_and_update" @keydown="clearError">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label for="">Mã</label>
                                <input type="text" class="form-control " v-model="workflowType.id"
                            ...
                            <div class="form-group">
                                <label for="">Tên</label>
                                <input type="text" class="form-control " v-model="workflowType.name"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control " v-model="workflowType.description"
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" required v-model="workflowType.active" name="status"
                                    id="status">
...
export default {
    ...
    methods: {
        ...
        save_and_update() {
            var page_url = this.page_url_workflow_type;// "/api/category/teams";
            if (this.edit === false) {
                fetch(page_url, {
                    method: "POST",
                    body: JSON.stringify(this.workflowType),
                    headers: {
                    ...
                    .then(data => {
                        ...
                        if (!data.data.errors) {
                            this.reset();
                            this.workflowTypes.push(data.data);
                ...
                //update
                fetch(page_url + "/" + this.workflowType.id, {
                    method: "PUT",
                    body: JSON.stringify(this.workflowType),
                    headers: {
                        Authorization: this.token,
                    ...
                    .then(data => {
                        console.log(data);
                        if (!data.data.errors) {
                            this.$set(this.workflowTypes, this.workflowType.index, { ...this.workflowType });
        ...
        editItem(item) {
            let index = this.workflowTypes.findIndex(i => {
                return i.id == item.id;
            });
            this.edit = true;
            this.workflowType.id = item.id;
            this.workflowType.active = item.active;
            this.workflowType.name = item.name;
            this.workflowType.description = item.description;
            this.workflowType.index = index;
        ...
        deleteItem(id) {
            ...
            if (confirm('Bạn muốn xoá?')) {
                fetch(`${this.page_url_workflow_type}/${id}`, {
                    method: 'delete',
                    headers: {
                        Authorization: this.token,
                        "content-type": "application/json"
        ...
        reset() {
            this.clearAllError();
            this.edit = false;
            for (let field in this.workflowType) {
                this.workflowType[field] = null;
            }
        },
        clearAllError() {
            this.errors = {};
    ...
    computed: {
        rows() {
            return this.workflowTypes.length;
        },
        placeholder() {
