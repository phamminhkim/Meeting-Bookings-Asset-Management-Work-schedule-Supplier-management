                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <b-table striped hover bordered responsive head-variant="light" small
                                        :fields="fields" :items="employeetypes" :filter="filter"
                    ...
                    <div class="modal-content">
                        <form @submit.prevent="submitDataForm" @keydown="clearError">
                            ...
                            <div class="modal-body">
                                <div class="form-group" v-if="employeetype.id">
                                    <label for="id">{{ $t('form.id') }}</label>
                                    <input v-model="employeetype.id" type="text" class="form-control"
                                ...
                                <div class="form-group">
                                    ...
                                    <input v-model="employeetype.code" type="text" class="form-control"
                                ...
                                <div class="form-group">
                                    ...
                                    <input v-model="employeetype.name" type="text" class="form-control"
...
export default {
    ...
    data() {
        return {
            ...
            page_url_employeetypes: "/api/category/employeetypes",
            fields: [
                {
                    key: 'id',
                    label: 'ID',
    ...
    methods: {
        fetchEmployeeType() {
            var page_url = this.page_url_employeetypes;
        ...
        submitDataForm() {
            var page_url = this.page_url_employeetypes;// "/api/category/departments";
            if (this.edit === false) {
                fetch(page_url, {
                    method: "POST",
                    body: JSON.stringify(this.employeetype),
                    headers: {
                    ...
                    .then(data => {
                        if (!data.data.errors) {
                            this.reset();
                            this.employeetypes.push(data.data);
                ...
                //update
                fetch(page_url + "/" + this.employeetype.id, {
                    method: "PUT",
                    body: JSON.stringify(this.employeetype),
                    headers: {
                        Authorization: this.token,
                    ...
                    .then(data => {
                        console.log(data);
                        if (!data.data.errors) {
                            this.$set(this.employeetypes, this.employeetype.index, { ...this.employeetype });
        ...
        editData(item) {
            let index = this.employeetypes.findIndex(i => {
                return i.id == item.id;
            });

            this.edit = true;
            this.employeetype.id = item.id;
            this.employeetype.code = item.code;
            this.employeetype.name = item.name;
            this.employeetype.index = index;
            $("#dataFormDialog").modal("show");
        },
        deleteData(id) {
            if (confirm('Bạn muốn xoá?')) {
                fetch(`${this.page_url_employeetypes}/${id}`, {
                    method: 'delete',
                    headers: {
                        Authorization: this.token,
                        "content-type": "application/json"
        ...
        reset() {
            this.clearAllError();
            this.edit = false;
            for (let field in this.employeetype) {
                this.employeetype[field] = null;
            }
        },
        clearAllError() {
            this.errors = {};
    ...
    computed: {
        ...
        rows() {
            return this.employeetypes.length;
        },
    },
};
