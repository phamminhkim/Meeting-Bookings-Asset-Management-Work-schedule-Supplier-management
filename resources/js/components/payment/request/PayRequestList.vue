    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-sm-6">
            <create-new-menu url="payment/requests?type=add&doctype=" module="PAYMENT"></create-new-menu>
            ...
            <div class="row" v-if="show_search" style="border: 1px solid #e5e5e5; border-radius: 5px">
              <div class="col-sm-12">
                ...
                <div class="form-group row">
                  ...
                  <div class="col-sm-10">
                    <treeselect id="document_type_id" :disabled="edit" :disable-branch-nodes="true"
                      v-model="form_filter.document_type_id" :options="tree_documentTypes" :multiple="true" required />
                  </div>
                </div>
                <div class="form-group row">
                  <label class="col-form-label-sm col-sm-2 col-form-label text-left text-md-right" for="">{{
              ...
              <div class="col-md-9">
                <div class="form-group row">
                  ...
                  <button type="button" class="btn btn-danger btn-sm ml-1 mt-1" :title="$t('form.cancel')"
                    ...
                    <i class="fas fa-window-close" :title="$t('form.cancel')"></i>
                    {{ $t("form.cancel") }}
                  </button>
                  <button type="button" class="btn btn-success btn-sm ml-1 mt-1" @click.prevent="update_paid()">
                    <i class="fas fa-bookmark"></i> {{ $t("form.paid") }}
                  </button>
                  <button type="button" class="btn btn-secondary btn-sm ml-1 mt-1" @click.prevent="notification_show()">
                    <i class="fas fa-bell"></i> {{ $t("form.reminder") }}
                  </button>
                  
                  <download-excel id="btnExcel" class="btn btn-info btn-sm ml-1 mt-1" 
                    :fields="fieldexcel" 
                     :before-generate = "startDownload"
                     :before-finish   = "finishDownload"
                    :fetch   = "fetchRequestExcel"
              ...
              <div class="row">
                <b-table striped hover responsive :bordered="true" head-variant="light" :sticky-header="true" small
                  ...
                  :filter-included-fields="[
                    ...
                    'receiver',
                    'amount',
                  ]" selectable ref="selectableTable" filter-debounce="1000" :fields="fields" @filtered="onFiltered">
                  ...
                  <template v-slot:cell(isRelease)="data">
                    <span v-if="data.item.isRelease == 'X'" class="badge bg-warning">
                      X
                    </span>
                  </template>
                  <template v-slot:cell(miss_invoice)="data">
                    <div style="text-align: center">
                      <input type="checkbox" v-model="data.item.miss_invoice"
                        @change.prevent="setMissInvoice($event, data.item.id)" />
                    </div>
                  </template>
                  <template v-slot:cell(printed)="data">
                    <div style="text-align: center">
                      <input type="checkbox" v-model="data.item.printed"
                        @change.prevent="setPrinted($event, data.item.id)" />
                    </div>
                  </template>
                  ...
                  <template #cell(date_receive_hard_doc)="data">
                    <div class="d-flex justify-content-between">
                      <span>{{ data.value | formatDateDB }}</span>
                      <a href="#" @click.prevent="change_date_receive_hard_doc(data.item)"><i
                  ...
                  <template #cell(document_type_id)="data">
                    <span>{{ get_document_type_name(data.value) }} </span>
                  </template>
                  ...
                  <template v-slot:cell(team_id)="data">
                    <div class="d-flex justify-content-between">
                      <a v-if="canviewteam == 1" :href="viewTeam(data.item.team_id)">{{
                      data.item.team_id
                      }}</a>
                      <a v-else> {{ data.item.team_id }}</a>
                    </div>
                  </template>
                </b-table>
                <div class="row">
...
export default {
  ...
  data() {
    return {
      variant_name: "payment_request_PayRequestList",
      requests: [],
      total: 0,
      per_page: 10,
      from: 1,
      ...
      locale_format: "de-DE",
      tableData: [],
      fileName: "DS_YCTT_" + moment(Date()).format("MM/DD/YYYY"),
      fieldexcel: {
        ...
        "Số tiền": "Số_tiền",
        "Tiền tệ": "Tiền_tệ",
        "Tên ngân hàng": "Tên_ngân_hàng",
        "Chi nhánh ngân hàng": "Chi_nhánh_ngân_hàng",
        "Số tài khoản": "Số_tài_khoản",
      ...
      fields: [
        ...
        {
          key: "company_id",
          label: this.$t("form.company_code"),
          sortable: true,
          stickyColumn: true,
          class: "text-nowrap",
        },
      ...
      edit: false,
      token: "",
      page_url_request: "/api/payment/requests",
  ...
  methods: {
    ...
    fetDocumentTypes_Tree() {
      // const this.token = "Bearer " + window.Laravel.access_this.token;
      var page_url = this.page_url_document_type + "?module=PAYMENT&type=tree_combobox"; 
      fetch(page_url, { headers: { Authorization: this.token } })
        .then((res) => res.json())
        .then((res) => {
          //console.log("Xin chao");
          this.tree_documentTypes = res.data;
        })
        .catch((err) => console.log(err));
    },
    fetVendor() {
      ...
      fetch(page_url, {
        ...
        .then((res) => {
          ...
          if (res.success == "1") {
            ...
            //     Tên_ngân_hàng: this.requests[i].bank_name,
            //     Chi_nhánh_ngân_hàng: this.requests[i].bank_branch,
            //     Số_tài_khoản: this.requests[i].bank_account,
            ...
            //     Mã_NS_TS: this.requests[i].gl_accounts,
            //   });
            // }
            // console.log(list);
            // this.tableData = list;
   ...
   finishDownload(){
    ...
    async  fetchRequestExcel(page, search) {
              ...
              let res = await response.json();
               ...
               if (res.canviewteam) {
                ...
                for (let i = 0; i < requests.length; i++) {
                  ...
                  var miss_invoice = "";
                  if (requests[i].miss_invoice == 0) miss_invoice = "Không";
                  else miss_invoice = "Có";

                  var hard_doc = "";
                  if (requests[i].date_receive_hard_doc == 0) hard_doc = "Không";
                  else hard_doc = "Có";
                  var printed = "";
                  if (requests[i].printed == 0) printed = "Chưa in";
                  else printed = "Đã in";

                  var send_date = "";
                  if (requests[i].send_date != null) {
                    send_date = moment(requests[i].send_date).format("MM/DD/YYYY");
                  } else {
                    send_date = "";
                  }
                  var method_payment = "";
                  if (requests[i].method_payment == 1) {
                    method_payment = "Tiền mặt";
                  } else {
                    method_payment = "Chuyển khoản";
                  }
                  ...
                  list.push({
                    Số_chứng_từ: requests[i].serial_num,
                    Trạng_thái: is_status,
                    Chờ_duyệt: requests[i].waitApprove,
                    Nợ_HĐ: miss_invoice,
                    Ngày_nhận_bản_cứng: hard_doc,
                    In: printed,
                    Phản_hồi: requests[i].noting,
                    Nội_dung: requests[i].content,
                    Số_tiền: requests[i].amount,
                    Tiền_tệ: requests[i].currency,
                    Tên_ngân_hàng: requests[i].bank_name,
                    Chi_nhánh_ngân_hàng: requests[i].bank_branch,
                    Số_tài_khoản: requests[i].bank_account,
                    Đơn_vị_cá_nhân_nhận_tiền: requests[i].money_receiver,
                    Thời_hạn_thanh_toán: requests[i].finish_date,
                    Hình_thức_thanh_toán: method_payment,
                    Ngày_gửi: send_date,
                    Loại_chứng_từ: this.get_document_type_name(
                      requests[i].document_type_id
                    ),
                    Người_tạo: requests[i].user.name,
                    Mã_số_nhân_viên: requests[i].user.username,
                    Mã_nhóm: this.get_group_name(requests[i].group_id),
                    Bộ_phận: this.get_department_name(requests[i].department_id),
                    Mã_bộ_phận: this.get_department_code(requests[i].department_id),
                    Công_ty: requests[i].company_id,
                    Mã_NS_TS: requests[i].gl_accounts,
                  });
                }
    ...
    viewRequest(id) {
      ...
      return "payment/requests?type=view&id=" + id;
    },
    viewTeam(id) {
      //window.location.href= "payment/requests?type=view&id="+id;
