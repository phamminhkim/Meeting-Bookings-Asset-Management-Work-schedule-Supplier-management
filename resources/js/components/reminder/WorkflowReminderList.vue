<template>
    <div class="card">
        <div  class="card-header" style="background-color: #E5E5E5;">
            <h6 class="card-title"><i class="fas fa-bell"></i> {{$t('form.reminder')}} / {{ $t('form.note') }}</h6>
            <div class="card-tools">
                <button class="btn btn-secondary btn-sm" :title="$t('form.add')" @click="show()"> <i class="fas fa-plus"></i>  </button>
            </div>
        </div>
        <div class="card-body"  >
            <b-table v-if="items && items.length > 0" small="small" :items="items" id="reminderRef" ref="reminderRef" thead-class="hidden_header" :responsive="true" :fields="fields">
           ...
           <div v-else class="d-flex justify-content-center">Không có ghi chú/nhắc nhở</div>
        </div>
