...
@section('content')
    ...
    @if($type == 'view')
        <record-detail title="{{ $menucurr->title }}" :form_title='"{{$form_title}}"' user_id='{{ Auth()->user()->id }}' :filter={{ json_encode($filter) }}>
        </record-detail>
    @else
        <record-list title="{{ $menucurr->title }}" user_id='{{ Auth()->user()->id }}' id=''></record-list>
        @endif
